...
#define SKETCH_NAME  "ELKv51"
#define PAIRED_WITH  "HVACv66"
// **************************************************************************
//
// ELKv51 -- Lake-only portrait per-CT display.
// Reviewed hardening: Frame B size guard, critical-section commit, and no alarm logging from ESP-NOW callback.
// Major protocol correction: Frame A is deprecated/unusable as a live ESP-NOW
// transport unit. ELK now ignores Frame A and accepts only smaller Frame B.
// Frame B carries packetNum, essential former-Frame-A display fields, and
// embedded CRC32. ELK verifies CRC before committing/displaying.
// Detection only: bad packets are logged/rejected; no automatic recovery action.
// Carries forward v47 fix: commit received Frame B (pendingExt -> receivedExt) and erase stale waiting text.
...
struct hvac_lake_extension {
  uint16_t magic;            // 0xE36B = Frame B sentinel
  uint8_t  version;
  uint8_t  reserved;
  ...
  // v51: Frame B is now the only live ELK transport packet. These fields
  ...
  // v51: integrity fields. crc32 covers bytes from struct start through the
...
volatile bool extAlarmPending = false;
static portMUX_TYPE extMux = portMUX_INITIALIZER_UNLOCKED;
...
// ================================================================
static inline void updateLinkQualityNow() {
...
void onDataReceive(const esp_now_recv_info *recv_info, const uint8_t *data, int data_len) {
  ...
  // v51: Frame A is deprecated/unusable as a live ESP-NOW transport packet.
  // Do not commit it and do not depend on it for display data.
  if (data_len == (int)sizeof(hvac_data)) {
    Serial.printf("[RX A] ignored deprecated Frame A len=%d\n", data_len);
    updateLinkQualityNow();
    return;
  }

  if (data_len == (int)sizeof(hvac_lake_extension)) {
    ...
    const uint32_t gotCrc = tmp.crc32;
    tmp.crc32 = 0;
    const size_t crcLen = offsetof(hvac_lake_extension, crc32);
    const uint32_t calcCrc = crc32_zlib((const uint8_t*)&tmp, crcLen);
    Serial.printf("[CRC] rcvd pktB=%lu crc=0x%08lX calc=0x%08lX size=%d crcLen=%u\n",
                  tmp.packetNum, (unsigned long)gotCrc, (unsigned long)calcCrc,
                  data_len, (unsigned)crcLen);

    if (gotCrc != calcCrc) {
      Serial.printf("[CRC ERROR] reject pktB=%lu got=0x%08lX calc=0x%08lX\n",
                    tmp.packetNum, (unsigned long)gotCrc, (unsigned long)calcCrc);
      updateLinkQualityNow();
      return;
    }
...
static void drawLakeHeader() {
  ...
  lcd.printf("%-19s", receivedExt.timeStr);
}

static void drawLakeStatusLine() {
  bool err = (receivedExt.systemError != 0) ||
  ...
  if (err) {
    const char* msg = (receivedExt.systemError != 0) ? receivedExt.errorMsg : receivedExt.first_error;
...
static void drawLakeDerivedLine() {
  ...
  lcd.printf("HDD %.1f  CDD %.1f  COP %.1f  Out %.0fF  Src %.1fF",
             receivedExt.hddToday, receivedExt.cddToday, cop,
             receivedExt.outdoorTemp, src);
}
...
void setup() {
  ...
  Serial.println("   Frame A: DEPRECATED/IGNORED  Frame B: CRC32 verified");
  Serial.println("===============================================\n");
  ...
  Serial.println("[ESP-NOW] Ready -- listening for " PAIRED_WITH " Frame B only (Frame A ignored)");
...
void loop() {
  ...
  // v51: Frame A is deprecated. dataReady should never be set by the RX path.
  if (dataReady) {
    dataReady = false;
  }
  ...
  if (extPendingReady) {
    ...
    alarmNow = extAlarmPending;
    extAlarmPending = false;
    portEXIT_CRITICAL(&extMux);
    extDataReady = true;
    dataReceived = true; firstDataReceived = true;
    if (lastReceivedPacketNum > 0) {
      if (receivedExt.packetNum > lastReceivedPacketNum)
        packetsLost = receivedExt.packetNum - lastReceivedPacketNum - 1;
      else if (receivedExt.packetNum < lastReceivedPacketNum) {
        packetsLost = 0; lastReceivedPacketNum = 0;
      } else packetsLost = 0;
    } else packetsLost = 0;
    lastReceivedPacketNum = receivedExt.packetNum;
    if (alarmNow) soundAlarm("HVAC_ERROR");
  }
